...
#' nuisance_cf_rf: This function creates the nuisance parameters p(x), mu(x), and mu_d(x)
#' using regression forests via cross-fitting using the \code{\link{grf}} package
#' 
#' Cross-fitting is necessary because it ensures that the estimation of these nuisance parameters and 
#' the IATEs (Individualized Average Treatment Effects) is done in different samples to avoid overfitting. 
#' Cross-fitting involves splitting the data into multiple folds, using one fold for estimating 
#' the nuisance parameters and another fold for estimating the IATEs. 
#'
#' @param y_tr Vector of outcome values in training data
#' @param d_tr Vector of treament indicators in training data
#' @param x_tr Matrix of covariates (N x p matrix) in training data. p is the same thing as Xs.
...
nuisance_cf_rf = function( y_tr, d_tr, x_tr, index,
                           current_seed
                           ) {
  ...
  for( i in 1:length( index ) ) {
    ...
    # Fit a regression forest model to estimate propensity scores based on the covariate data 
    # in x_tr and the treatment indicators in d_tr for a particular fold specified by index[[i]]
    fit_p = do.call( regression_forest, c( list( X = x_tr[-index[[i]],,drop=F],
                                                 Y = d_tr[-index[[i]]] ),
                                           tune.parameters = 'all', num.threads = 1, seed = current_seed 
                                           ) )
    
    # Predict the propensity scores for the current fold's test data and 
    # store them in the np matrix
    np[index[[i]],1] = predict( fit_p, x_tr[index[[i]],,drop=F] )$prediction
    
    
    # Fit a regression forest model to estimate the expected outcome based on the covariate data 
    # in x_tr and the outcome in y_tr for a particular fold specified by index[[i]]
    fit_y = do.call( regression_forest, c( list( X = x_tr[-index[[i]],,drop=F], #TODO:: Ask Luke why i was 1 in the old code
                                                 Y = y_tr[-index[[i]]] ),
                                           tune.parameters = 'all', num.threads = 1, seed = current_seed
                                           ) )
    ...
    # store them in the np matrix 
    np[index[[i]],2] = predict( fit_y, x_tr[index[[i]],,drop=F] )$prediction
    
    
    # Fit a regression forest model to estimate the expected outcome under the control group 
    ...
    fit_y0 = do.call( regression_forest, c( list( X = x_tr[-index[[i]],,drop=F][d_tr[-index[[i]]] == 0,,drop=F],
    ...
    np[index[[i]],3] = predict( fit_y0, x_tr[index[[i]],,drop=F] )$prediction
    
    
    # Fit a regression forest model to estimate the expected outcome under the treatment  group 
    ...
    fit_y1 = do.call( regression_forest, c ( list( X = x_tr[-index[[i]],,drop=F][d_tr[-index[[i]]] == 1,,drop=F],
    ...
    # for the current fold's test data and store them in the np matrix 
    np[index[[i]],4] = predict( fit_y1,x_tr[index[[i]],,drop=F] )$prediction
    
    #TODO:: MCKnaus - "Think about predicting also for the other treatment category in the other fold and take the average"
  }
