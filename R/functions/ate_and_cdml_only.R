...
#' @param scen_outcome_index outcome_index to filter from all_scenarios
#' @param queen1 first queen you want to plot
#' @param queen2 second queen you want to plot
#' 
#' 

plot_two_queens = function(scen_dataset, scen_outcome_index, queen1 , queen2){
  ...
  scen_max = scenario_maximums%>%filter(dataset == scen_dataset & outcome_index == scen_outcome_index)%>% select(max_agg) %>% unlist()
  scen_max_bias = scenario_maximums%>%filter(dataset == scen_dataset & outcome_index == scen_outcome_index)%>% select(max_bias_agg) %>% unlist()
  scen_max_se = scenario_maximums%>%filter(dataset == scen_dataset & outcome_index == scen_outcome_index)%>% select(max_se_agg) %>% unlist()
  scen_breaks = scenario_maximums%>%filter(dataset == scen_dataset & outcome_index == scen_outcome_index)%>% select(breaks_agg) %>% unlist()
