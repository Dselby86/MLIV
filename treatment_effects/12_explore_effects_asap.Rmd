...
```{r}
source(here::here("treatment_effects/11-Calculate CATES_asap.R"))
...
tab_binary <- dat %>%
  group_by(quant_b) %>%
  summarize(BLFEMALE = mean(BLFEMALE, na.rm = TRUE),
            BLAGENUM = mean(BLAGENUM, na.rm = TRUE),
            BLWORK = mean(BLWORK, na.rm = TRUE),
            BLCHLD = mean(!!sym(parent), na.rm = TRUE)
            )

overall_b  <- dat %>%
  summarize(
    quant_b = "Overall",
    BLFEMALE = mean(BLFEMALE, na.rm = TRUE),
    BLAGENUM = mean(BLAGENUM, na.rm = TRUE),
    BLWORK = mean(BLWORK, na.rm = TRUE),
    BLCHLD = mean(!!sym(parent), na.rm = TRUE)
    )
