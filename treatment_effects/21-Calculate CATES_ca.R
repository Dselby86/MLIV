...
source(here::here("treatment_effects/20-call_sim_w_ca.R")) # Most Recent Simulation
...
#######################################
dat_raw <- ca_subset_imputed
treatment <- ca_treatment
y_binary <- "BMEFTMEDIAN"
y_cont <- "Y18JBERNA_06"
cv <- ca_large_set
factor_vars <- c("ETHNIC", "READCAT", "MATHCAT", "PARENT", "edfath", "edmoth", "HRSTV", "HRSHW", "PARWORK", "PSEDEXP", "MOVED", "OFFICE")
...
dat_ca <- merge(dat, cate_agg, by = "id")
